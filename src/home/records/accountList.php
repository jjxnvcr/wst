...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            <button onclick="window.location.href = 'add/addAccount.html';">
        ...
        <div class="table-wrapper">
            <table>
                ...
                $res = query($conn, "SELECT * FROM `useraccount`");
                ...
                for ($i = 0; $i < mysqli_num_rows($res); $i++) {
                    ...
                    echo "<td>" . $row[3] . "</td>";
                    echo "<td>" . $row[4] . "</td>";
                    echo "<td>" . str_repeat("*", strlen($row[5])) . "</td>";

                    $fixedSurname = lcfirst(str_replace(" ", "", $row[1]));
                    
                    echo "<td>";
                        echo(
                            "<button onclick=\"window.location.href = '../records/view/viewAccount.php?id={$row[0]}';\">View</button>"
                        );
                        echo(
                            "<button onclick=\"window.location.href = '../records/edit/editAccount.php?id={$row[0]}';\">Edit</button>"
                        );
                        echo(
                            "<button onclick=\"window.location.href = '../records/delete/deleteAccount.php?id={$row[0]}';\">Delete</button>"
                        );
                    echo "</td>";
                    echo "</tr>";
                }
